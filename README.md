<h1 align="center">local-market-scanner</h1>

<p align="center">
  Bursa'daki hangi sağlık işletmesinin web sitesi yok, sosyal medyası yok,<br>
  ama ulaşılabilir bir telefonu var? Tek tıkla, kamuya açık veriden, ölçülebilir bir cevap.<br>
  <b>989 işletme tarandı → 785 nitelikli aday → 1 dakika 15 saniye.</b>
</p>

<p align="center">
  <a href="https://github.com/umutseve4/local-market-scanner/actions/workflows/ci.yml"><img src="https://github.com/umutseve4/local-market-scanner/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
  <a href="https://github.com/umutseve4/local-market-scanner/actions/workflows/scan.yml"><img src="https://github.com/umutseve4/local-market-scanner/actions/workflows/scan.yml/badge.svg" alt="scan"></a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/canl%C4%B1%20ko%C5%9Fuda%20taranan-989%20i%C5%9Fletme-FF4D4F?style=flat-square" alt="989 işletme">
  <img src="https://img.shields.io/badge/test-179%20offline-FF4D4F?style=flat-square" alt="179 test">
  <img src="https://img.shields.io/badge/CLI%20komutu-8-FF4D4F?style=flat-square" alt="8 komut">
</p>

<p align="center">
  <b><a href="https://github.com/umutseve4/local-market-scanner/actions/workflows/scan.yml">▶ Actions → scan → Run workflow ile bulutta çalıştır</a></b>
</p>
...
`doctor` ön kontrolü → `scan --track` (SQLite'a koşu kaydı) → `validate` (veri
sözleşmesi) → `brief` → özet + artifact.

> **Canlı doğrulama (2026-08-16):** GitHub Actions üzerinde gerçek Overpass API'ye
> karşı uçtan uca koştu — **989 işletme tarandı, 785 nitelikli lead** üretildi
> (1m 15s, `scan #2`).
...
**Düşük skor = zayıf dijital varlık = daha iyi aday.** Ağırlıklar
`src/lms/models.py` içinde tek bir yerde tanımlıdır; kara kutu değildir,
değiştirilebilir ve testlerle sabitlenmiştir.
...
Çıktı, elle doğrulanması gereken bir **aday listesidir**. Aşağıdakiler gerçek
kısıtlardır, ileride kapatılacak "eksikler" değil:
...
**Hukuki uyarı (KVKK / İYS).** Sadece kamuya açık veri toplanır. Kişisel veri
(isim, telefon) saklandığı anda **KVKK** yükümlülüğü doğar: amaç sınırlaması,
saklama süresi, silme talebine yanıt. İzinsiz toplu ticari e-posta/SMS göndermek
**İYS** mevzuatını ihlal eder. **Bu araç iletişim göndermez, sadece liste üretir;
iletişim kurmak kullanıcının sorumluluğundadır.** Google Maps HTML'i **kazınmaz** —
bu Google Hizmet Şartları'nı ihlal eder. `.env` ve üretilen `data/*.csv`,
`data/*.sqlite3` dosyaları `.gitignore` içindedir. Ayrıntılar:
[SECURITY.md](SECURITY.md).

Veri: [OpenStreetMap](https://www.openstreetmap.org/copyright), Overpass API
üzerinden. OSM verisi **ODbL** ile lisanslıdır; türetilmiş veriyi yayımlarsan atıf
ve lisans yükümlülüğün vardır. Overpass genel sunucusu ortak bir kaynaktır:
gereksiz tekrarlı sorgu atma, sonuçları yerelde sakla.
...
<details>
<summary><b>Yerel kurulum ve 9 kullanım senaryosu</b></summary>
...
`Makefile` aynı komutları kısayola bağlar: `make scan`, `make leads`, `make brief`,
`make doctor`, `make test`, `make lint`, `make coverage`.

`brief` her aday için tek ekranda: eksik dijital varlıklar, önerilen teklif ve
görüşmeyi açacak bir cümle üretir. Fiyat aralıkları `src/lms/outreach.py` içinde
tek yerdedir.
...
Geçersiz bir değer (örn. `MAX_RETRIES=many`) sessizce yok sayılmaz; `ConfigError`
fırlatılır ve çıkış kodu `1` olur.

</details>

<details>
<summary><b>Test dağılımı — 179 test, ağ erişimi yok</b></summary>
...
Overpass yanıtı `tests/fixtures/overpass_sample.json` dosyasından okunur; HTTP
katmanı `FakeSession`/`FakeResponse` ile taklit edilir.
...
Geliştirme akışı: [CONTRIBUTING.md](CONTRIBUTING.md) ·
Sır yönetimi ve KVKK: [SECURITY.md](SECURITY.md) ·
